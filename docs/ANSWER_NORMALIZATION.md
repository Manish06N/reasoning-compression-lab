...
That path is the official QRM LightEval task (`lighteval_custom/tasks/reasoning.py`) plus the LightEval `[math]` extra (math-verify). A pair of completions *agree* when the campaign evaluator would have given both the same extractive-match decision against a shared reference string — equivalently, when their extracted answers are equivalent under that judge.
