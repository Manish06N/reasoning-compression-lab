#!/usr/bin/env bash
# ==============================================================================
# Submit Breadth Evaluation Campaign (GSM8K or GPQA-Diamond)
...
TASK="${1:-${QRM_DATASET:-GSM8K}}"
CAMPAIGN_DATE="${CAMPAIGN_DATE:-$(date +%Y-%m-%d)}"

if [[ "$TASK" == "GPQA-Diamond" || "$TASK" == "gpqa" ]]; then
  TASK_NAME="GPQA-Diamond"
  MAX_SAMPLES=198
  OUTPUT_ROOT="${QRM_OUTPUT_ROOT:-$QR/outputs-hpc-breadth-gpqa-${CAMPAIGN_DATE}}"
else
  TASK_NAME="GSM8K"
  MAX_SAMPLES=1319
...
echo "======================================================================"
echo " Launching Paper 1 Breadth Campaign Pipeline: $TASK_NAME (n=$MAX_SAMPLES)"
...
submit_chained_job() {
  ...
  local seed="$3"
  local prev_job_id="$4"
  local max_samples="${5:-$MAX_SAMPLES}"
...
PREFIX="$(echo "$TASK_NAME" | tr '[:upper:]' '[:lower:]' | cut -d'-' -f1)"

echo ""
echo "--- Submitting Channel 1: Qwen-7B ($TASK_NAME) Chained Pipeline (1 GPU) ---"

# Seed 42
Q1=$(submit_chained_job "${PREFIX}-qwen-bf16-s42" "$QWEN_BF16" 42 "none")
echo "  [Qwen 1] BF16  (Seed 42): $Q1 (Initial Active)"
Q2=$(submit_chained_job "${PREFIX}-qwen-fp8-s42"  "$QWEN_FP8"  42 "$Q1")
echo "  [Qwen 2] FP8   (Seed 42): $Q2 (Chained after $Q1)"
Q3=$(submit_chained_job "${PREFIX}-qwen-awq4-s42" "$QWEN_AWQ4" 42 "$Q2")
echo "  [Qwen 3] AWQ-4 (Seed 42): $Q3 (Chained after $Q2)"
Q4=$(submit_chained_job "${PREFIX}-qwen-gptq4-s42" "$QWEN_GPTQ4" 42 "$Q3")
echo "  [Qwen 4] GPTQ4 (Seed 42): $Q4 (Chained after $Q3)"

# Seed 43
Q5=$(submit_chained_job "${PREFIX}-qwen-bf16-s43" "$QWEN_BF16" 43 "$Q4")
echo "  [Qwen 5] BF16  (Seed 43): $Q5 (Chained after $Q4)"
Q6=$(submit_chained_job "${PREFIX}-qwen-fp8-s43"  "$QWEN_FP8"  43 "$Q5")
echo "  [Qwen 6] FP8   (Seed 43): $Q6 (Chained after $Q5)"
Q7=$(submit_chained_job "${PREFIX}-qwen-awq4-s43" "$QWEN_AWQ4" 43 "$Q6")
echo "  [Qwen 7] AWQ-4 (Seed 43): $Q7 (Chained after $Q6)"
Q8=$(submit_chained_job "${PREFIX}-qwen-gptq4-s43" "$QWEN_GPTQ4" 43 "$Q7")
echo "  [Qwen 8] GPTQ4 (Seed 43): $Q8 (Chained after $Q7)"

# Seed 44
Q9=$(submit_chained_job "${PREFIX}-qwen-bf16-s44" "$QWEN_BF16" 44 "$Q8")
echo "  [Qwen 9] BF16  (Seed 44): $Q9 (Chained after $Q8)"
Q10=$(submit_chained_job "${PREFIX}-qwen-fp8-s44" "$QWEN_FP8" 44 "$Q9")
echo "  [Qwen 10] FP8  (Seed 44): $Q10 (Chained after $Q9)"
Q11=$(submit_chained_job "${PREFIX}-qwen-awq4-s44" "$QWEN_AWQ4" 44 "$Q10")
echo "  [Qwen 11] AWQ4 (Seed 44): $Q11 (Chained after $Q10)"
Q12=$(submit_chained_job "${PREFIX}-qwen-gptq4-s44" "$QWEN_GPTQ4" 44 "$Q11")
echo "  [Qwen 12] GPTQ4 (Seed 44): $Q12 (Chained after $Q11)"

echo ""
echo "--- Submitting Channel 2: Llama-8B ($TASK_NAME) Chained Pipeline (1 GPU) ---"

# Seed 42
L1=$(submit_chained_job "${PREFIX}-llama-bf16-s42" "$LLAMA_BF16" 42 "none")
echo "  [Llama 1] BF16  (Seed 42): $L1 (Initial Active)"
L2=$(submit_chained_job "${PREFIX}-llama-fp8-s42"  "$LLAMA_FP8"  42 "$L1")
echo "  [Llama 2] FP8   (Seed 42): $L2 (Chained after $L1)"
L3=$(submit_chained_job "${PREFIX}-llama-awq4-s42" "$LLAMA_AWQ4" 42 "$L2")
echo "  [Llama 3] AWQ-4 (Seed 42): $L3 (Chained after $L2)"
L4=$(submit_chained_job "${PREFIX}-llama-gptq4-s42" "$LLAMA_GPTQ4" 42 "$L3")
echo "  [Llama 4] GPTQ4 (Seed 42): $L4 (Chained after $L3)"

# Seed 43
L5=$(submit_chained_job "${PREFIX}-llama-bf16-s43" "$LLAMA_BF16" 43 "$L4")
echo "  [Llama 5] BF16  (Seed 43): $L5 (Chained after $L4)"
L6=$(submit_chained_job "${PREFIX}-llama-fp8-s43"  "$LLAMA_FP8"  43 "$L5")
echo "  [Llama 6] FP8   (Seed 43): $L6 (Chained after $L5)"
L7=$(submit_chained_job "${PREFIX}-llama-awq4-s43" "$LLAMA_AWQ4" 43 "$L6")
echo "  [Llama 7] AWQ-4 (Seed 43): $L7 (Chained after $L6)"
L8=$(submit_chained_job "${PREFIX}-llama-gptq4-s43" "$LLAMA_GPTQ4" 43 "$L7")
echo "  [Llama 8] GPTQ4 (Seed 43): $L8 (Chained after $L7)"

# Seed 44
L9=$(submit_chained_job "${PREFIX}-llama-bf16-s44" "$LLAMA_BF16" 44 "$L8")
echo "  [Llama 9] BF16  (Seed 44): $L9 (Chained after $L8)"
L10=$(submit_chained_job "${PREFIX}-llama-fp8-s44" "$LLAMA_FP8" 44 "$L9")
echo "  [Llama 10] FP8  (Seed 44): $L10 (Chained after $L9)"
L11=$(submit_chained_job "${PREFIX}-llama-awq4-s44" "$LLAMA_AWQ4" 44 "$L10")
echo "  [Llama 11] AWQ4 (Seed 44): $L11 (Chained after $L10)"
L12=$(submit_chained_job "${PREFIX}-llama-gptq4-s44" "$LLAMA_GPTQ4" 44 "$L11")
echo "  [Llama 12] GPTQ4 (Seed 44): $L12 (Chained after $L11)"

echo ""
echo "======================================================================"
echo " Pipeline Launch Complete: 2 Channels, 24 Jobs, 2 Active GPUs"
echo " Monitoring command: squeue -u \$USER"
echo "======================================================================"
