...
(C1)~\textbf{One robust accuracy drop.} Llama AWQ-4 is the only contrast that is Holm-significant on both MATH-500 and GSM8K; maj@5 recovers about half of it. AWQ results are properties of the \texttt{jakiAJK} artifacts run in \texttt{float16} on the GEMM \texttt{awq} kernel, not of the AWQ method.
...
\begin{table}[t]
\centering
\caption{Qwen-7B numbers reported by QRM beside this pinned grid. Cells are matched by benchmark and format. QRM quantized its own AWQ-4 model, whereas this grid evaluates the public \texttt{jakiAJK} upload, so the AWQ rows compare different artifacts. The GPQA row is not a certified paired contrast (Appendix Section~\ref{sec:prompt}). The discrepancy is checkpoint- and stack-sensitive measurement, not a superiority claim.}
...
\begin{table}[t]
\centering
\caption{GPQA-Diamond pass@1 versus matched BF16 ($n=198$, 3 seeds). The rows are descriptive. Choice order is not certified as paired across checkpoints (Appendix Section~\ref{sec:prompt}). The Qwen AWQ-4 $p$ uses $B=10^6$ (other rows $B=10{,}000$); it is borderline under Holm-6 (adjusted $0.049$ bootstrap, $0.066$ permutation) and not significant under Holm-18, even if the pairing is taken at face value. It is not a headline claim. border.: borderline; n.s.: not significant.}
...
Near-cap rows are very likely truncations. Of the 140 MATH-500 near-cap rows, 121 lie between $32{,}600$ and $32{,}739$ tokens, the band where $32{,}768$ minus a typical prompt falls. Of the 57 GSM8K near-cap rows, 55 lie in that band. Only 24 MATH-500 rows and no GSM8K rows fall between the lowest possible cap ($31{,}957$) and $32{,}500$. An exact per-item truncation count cannot be obtained by joining public prompt lengths on row position, because the compact JSON stores items in the campaign pipeline order, not the public release order. It requires the prompt text stored in the campaign output files. On MATH-500, 120 of 134 unboxed rows are near-cap. A missing box on MATH is usually a long trace. It is not the 4-bit accuracy gap: unboxed rows that are still scored correct are rare (at most two per seed file), and boxed rates are $98.96$--$99.68\%$. GSM8K is similar (52 of 66 unboxed rows are near-cap; boxed rates $99.70$--$99.87\%$). GPQA is different: $4{,}746$ of $4{,}752$ rows are unboxed, and only 12 are near-cap. GPQA extractive match is not a boxed-answer grade, and the long-tail proxy does not explain it. Native stop reasons were not logged, so near-cap is a length proxy, not a \texttt{finish\_reason} count.
...
A per-level breakdown needs each row's MATH-500 \texttt{level}. The compact JSON stores items in the campaign pipeline order, which differs from the public release order: among correct rows with integer gold answers, the recovered prediction equals the public gold at the same position in only 118 of 11{,}382 cases. A positional join would therefore assign the wrong level to almost every row, so we do not report a per-level breakdown. The serving subsets are unaffected, because they were drawn by matching prompt text (Section~\ref{sec:measured-serving}).
...
\begin{enumerate}
    ...
    \item \textbf{Pathology metrology.} The loop detector is identical-word runs $\ge 20$. Phrase-level cycles and \texttt{finish\_reason} were not saved. Truncation is proxied by near-cap length. A per-item recount needs the campaign prompt text, because compact JSON rows are in pipeline order rather than public release order. GPQA prompts were not reconstructed, because the letter order is a process-RNG draw.
    \item \textbf{Item order.} Compact JSON rows follow the campaign pipeline order. All paired analyses are within that order and are unaffected: the eight cells agree item by item. Joins against public per-item metadata (difficulty level, prompt length) require the campaign prompt files and are not reported.
