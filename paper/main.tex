...
\title{\textbf{Evaluation-Target Instability in Quantized Reasoning Checkpoints: One Stack, Many Rankings}}
...
Three findings follow. MATH-500 FP8--BF16 pass@1 differences are $+0.40$ and $+0.28$ percentage points; item-level 95\% intervals include zero, and the 90\% intervals extend to $+1.12$ and $+1.28$\,pp, so a $\pm 1$\,pp equivalence test is not passed. Gold-free 5/5 answer agreement on MATH-500 has selective risk at most $0.27\%$, while a one-sample length rule at the same coverage has selective risk of $1.6$--$4.6\%$. Subset GPU-seconds and a campaign-length cost (full-grid mean tokens divided by measured tok/s) do not produce the same point order: on the 20-prompt Condition~A draw, Llama FP8 averages about $7{,}288$ output tokens per query, against $4{,}551$ on the full MATH-500 grid. Timing intervals cover wall-clock repeats of one token sequence. They do not cover that length draw.
\end{abstract}
...
\begin{itemize}
    \item \textbf{RQ1.} Do the evaluated quantized checkpoints differ in \emph{pass@1} from matched BF16, with item-level uncertainty?
...
(C1)~\textbf{Pinned evaluation protocol.} NVIDIA A100-80GB; vLLM~0.7.0 eager; public DeepSeek-R1-Distill checkpoints; recorded activation dtype and kernel per cell; item-level bootstrap; Holm correction; TOST $\pm 1$\,pp; and pass@1 reported beside maj@5.
(C2)~\textbf{Estimator-sensitive point orders.} Pass@1, maj@5, correctness-conditioned length, subset GPU-seconds, and campaign-length cost do not share one point order. Several of those differences are ties or a single length draw on a 20-prompt subset. We report rank frequencies where the frozen rows support them.
(C3)~\textbf{Artifact scope.} Both AWQ-4 checkpoints are one community upload (\texttt{jakiAJK}). FP8 and GPTQ-4 are RedHatAI artifacts. Format is collinear with publisher, calibration set, and toolchain, so an AWQ-versus-GPTQ gap is not a method comparison. The released grid supports reanalysis of these checkpoints.
...
Existing studies evaluate quantization accuracy, throughput, or individual reasoning behaviors. Kurtic et al.\ already show that format recommendations change between synchronous decode and continuous batching. Our Condition~A versus Condition~B contrast is a pinned-stack sensitivity on these public checkpoints, not a second batching law. The shared-$65$ tok/s proxy assumes one throughput for every format, so it ranks checkpoints by mean tokens divided by pass@1; it is a sensitivity, not a serving measurement. Appendix Table~\ref{tab:related} places this grid against those studies. The unit of comparison is the public checkpoint $\times$ pinned stack $\times$ evaluation target.

\begin{table}[t]
\centering
\caption{Qwen-7B numbers reported by QRM beside this pinned grid. Cells are matched by benchmark and format. The discrepancy is checkpoint- and stack-sensitive measurement, not a superiority claim.}
...
The FP8 and GPTQ-4 cells are public RedHatAI compressed-tensors artifacts evaluated as A100 Marlin weight-only kernels. These are not Red Hat production-deployment measurements. We do not rerun SmoothQuant~\cite{xiao2023smoothquant}. Lotfi et al.~\cite{lotfi2026overthinking}, Li et al.~\cite{li2025quantmeets}, Helcig et al.~\cite{helcig2026slq}, Zhong et al.~\cite{zhong2025quantized}, and Ayadi et al.~\cite{ayadi2026reliability} study overthinking, reasoning degradation, statistically lossless quantization, calibration, and reliability scaling; we do not re-estimate those quantities. Self-consistency~\cite{wang2022selfconsistency} and G-Pass@$k$~\cite{liu2025gpass} are different estimands from the unique-mode rule in Section~\ref{sec:modal}. We report a historical shared-$65$ tok/s token proxy and an aggregate hybrid Cost-of-Pass proxy; neither is Erol et al.'s per-problem estimator.
...
\begin{table}[t]
\centering
\caption{Activation dtype and vLLM kernel. Accuracy dtype is the 56k campaign. Serving dtype is the confirmation timer. AWQ uses the GEMM awq kernel, not awq-marlin.}
\label{tab:dtype}
\small
\begin{tabular}{llll}
\toprule
...
GPTQ-4 & \texttt{bfloat16} & compressed-tensors & \texttt{float16} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Pathology.}
A row is a \emph{loop} if the longest consecutive identical-word run is $\ge 20$ (Unicode word tokens, case-folded). The launcher sets both \texttt{max\_model\_len} and \texttt{max\_new\_tokens} to $32{,}768$. In vLLM the first bound is prompt plus output, so the effective output cap is $32{,}768$ minus that item's prompt length. A row is counted as an exact cap hit only when the re-encoded completion is $\ge 32{,}768$. The observed maximum is $32{,}737$, and that count is $0$. Compact JSON has no \texttt{finish\_reason} and no per-item prompt length, so near-cap rows ($\ge 32{,}500$ tokens) are not reclassified as truncations. A longer GPQA prompt can truncate below that $32{,}500$ threshold. The \texttt{boxed} flag is a string search for \texttt{\textbackslash boxed}, which is separate from extractive match.
...
\end{equation}
divided by campaign pass@1, with $\tau = 65$ tok/s and $R=\$1.50$ per A100-hour. The constant $65$ matches none of the measured tok/s values ($43.9$--$82.7$ on Condition~A, $253$--$488$ on Condition~B). It is an unmeasured shared placeholder from an earlier draft of this proxy. Because $\tau$ and $R$ are shared, the ranking equals the ranking by $\bar{T}/\mathrm{pass@1}$. Changing $65$ rescales the dollars and does not reorder checkpoints. Appendix Table~\ref{tab:economics} keeps the historical dollars. Primary cost evidence is Table~\ref{tab:serving-main} together with the campaign-length sensitivity in Table~\ref{tab:length-cost}.
...
\begin{table}[t]
\centering
\caption{GPQA-Diamond item-level pass@1 versus matched BF16 ($n=198$, 3 seeds). Holm over six contrasts. sig.: Holm-significant at $\alpha=0.05$; n.s.: not significant. The Qwen AWQ-4 row is Holm-6 significant and Holm-18 not significant (Appendix Table~\ref{tab:holm18}). It is not a headline claim.}
...
\label{sec:pathology}
Across 88 runs the validator records \textbf{25} repetition-flagged completions ($0.044\%$ of 56{,}408) and \textbf{0} rows whose re-encoded length is $\ge 32{,}768$. Loops occur in BF16 as well as quantized cells (Llama MATH: 3/3/3/2 for BF16/FP8/AWQ/GPTQ). Three completions exceed 10{,}000 identical consecutive words. Near-cap completions ($\ge 32{,}500$ tokens) total 209; on MATH-500, Qwen AWQ-4/GPTQ-4 have 25 and 24 versus BF16's 14. Those near-cap counts can include truncations at \texttt{max\_model\_len} minus the prompt, and they can miss shorter GPQA truncations. Native stop reasons were not logged.

On MATH-500, completions containing \texttt{\textbackslash boxed} are $98.96$--$99.68\%$ of each cell ($13$--$26$ unboxed rows out of $2{,}500$). Unboxed rows that are still scored correct are rare (at most two per seed file). The 4-bit MATH gaps are parsed wrong answers, not missing boxes. GSM8K boxed rates are $99.70$--$99.87\%$. GPQA is different: $0$--$2$ of $198$ rows per seed contain \texttt{\textbackslash boxed}, in every format including BF16, while pass@1 remains about $41$--$53\%$. GPQA extractive match is not a boxed-answer grade.
...
Table~\ref{tab:difficulty}: Level~1 is easy for every format in this sample. Llama AWQ-4 has the lowest Llama pass@1 on Levels 2--4. Level~2 is harder than Level~5 for both BF16 backbones (Qwen $89.11\%$ vs.\ $92.69\%$; Llama $84.67\%$ vs.\ $88.06\%$). The recovered answer file has no level field, so this split was not re-audited for extraction failures concentrated in Level~2. The non-monotonicity is a property of these labels under the campaign grader.
...
The printed dollar intervals multiply wall-clock repeats by a pass@1 bootstrap. Repeats share seed $20260816$, so they do not include length variance. With $R=3$ we report the observed tok/s range rather than a percentile story: Qwen BF16 Condition~A is $43.88$--$43.96$; Qwen FP8 Condition~B is $350.68$--$546.26$. Qwen FP8 Condition~B tok/s of $449.79$ is the mean of five ratios. Mean GPU-s/q is $8.64$, and $3{,}738/8.64 \approx 432.5$ tok/s. The two aggregations are not the same. Appendix Table~\ref{tab:fp8-reps} lists the five repeats. Llama FP8 and AWQ-4 Condition~A both print as $\$0.0431$ $[0.0415, 0.0450]$. The unrounded costs are $0.04308$ and $0.04313$. They are not a copied cell.

Table~\ref{tab:length-cost} substitutes campaign mean tokens for the subset length and keeps the measured tok/s. Under that sensitivity, Qwen GPTQ-4 is cheapest on Condition~A and tied with FP8 on Condition~B, so the AWQ/GPTQ reversal between A and B disappears. Llama FP8 is cheapest on both conditions and on the token proxy. An item bootstrap of campaign tokens and pass@1, with tok/s held fixed ($B=10{,}000$), gives Llama FP8 rank~1 on both campaign-length costs in all $10{,}000$ draws. Qwen GPTQ-4 is rank~1 on Condition~A in $99.9\%$ of draws and on Condition~B in $59.6\%$, with FP8 taking the other $40.4\%$. Kendall's $\tau$ between the Qwen pass@1 order and the Condition~A campaign-length order is $-0.33$. Measured tok/s still depends on the subset's length, so Table~\ref{tab:length-cost} is not a final serving measurement.

Qwen BF16 decodes at $43.92$ tok/s on Condition~A and $205.41$ tok/s in the fixed-512 microbenchmark. Llama BF16, on the other host, reaches $72.34$ and $362.01$. The fixed-length gap remains, so it is not the Condition~A length draw. Every Qwen saving in Table~\ref{tab:serving-main} ($-36.0\%$ to $-45.9\%$) is relative to that Qwen BF16 baseline on \texttt{ragpu003}. The Qwen FP8 slow/mid/fast regimes were also on that host. Clocks were not logged. These FP8 numbers are A100 Marlin W8A16, not Hopper W8A8. Peak allocated VRAM is $53.9$--$56.1$\,GB, the $0.75$ engine pool. Figure~\ref{fig:condB-scatter} is not a pooled trade-off plot. Condition~A latency is in Appendix Table~\ref{tab:serving-single}.
...
\section{Discussion}
Table~\ref{tab:summary} collects the quantities a practitioner has to assemble from the earlier tables. MATH-500 FP8--BF16 intervals include zero. The $\pm 1$\,pp TOST fails because those 90\% intervals reach $+1.12$ and $+1.28$\,pp. Llama AWQ-4 is the MATH and GSM8K accuracy drop in this grid. Qwen AWQ-4 stays near BF16 on those two tasks. Its GPQA contrast is in Table~\ref{tab:gpqa-contrasts} and is not a headline. Qwen 4-bit Both-OK traces are longer. The BF16-correct length column is positive in every contrast because it includes quantized failures. The quant-correct column does not have that sign.

maj@5 removes the Qwen AWQ-4 pass@1 gap and about half of the Llama AWQ-4 gap. Modal 5/5 coverage for Llama AWQ-4 is $6.0$\,pp lower than BF16. A one-sample length threshold at the same coverage has higher selective risk (Table~\ref{tab:length-abstain}). The five-sample rule earns that lower risk at about five times the tokens.

Subset GPU-seconds and campaign-length seconds do not share a point order. The subset order is real for the measured draws. It is also sensitive to a length variance the printed intervals do not cover. Eager execution and the Qwen host are part of that serving result. A point order among tied or non-significant cells is not, by itself, evidence that rankings are unstable.
...
\begin{enumerate}
    ...
    \item \textbf{Pathology metrology.} The loop detector is identical-word runs $\ge 20$. Phrase-level cycles and \texttt{finish\_reason} were not saved. The exact-cap count is re-encoded length, not \texttt{max\_model\_len} minus the prompt. Near-cap is a proxy and can miss GPQA truncations.
...
\section{Conclusion}
Under this pinned A100 / vLLM~0.7.0 eager stack, pass@1 does not order the eight checkpoints by itself. The only Holm-significant MATH drop is Llama AWQ-4 ($-2.76$\,pp); maj@5 cuts that gap to about $-1.4$\,pp and removes the Qwen AWQ-4 gap. Gold-free 5/5 agreement has selective risk at most $0.27\%$, against $1.6$--$4.6\%$ for a one-sample length rule at the same coverage. The BF16-correct length increase is positive in every contrast because it conditions on quantized failures; the quant-correct mirror does not keep that sign. Subset GPU-second rankings disagree with a campaign-length cost that uses the same measured tok/s, and the subset intervals do not cover length variance. Qwen savings are relative to a BF16 baseline on \texttt{ragpu003}. AWQ-4 results are properties of the \texttt{jakiAJK} artifacts, run in \texttt{float16}, not a property of the AWQ method.
...
CPU-reproducible from compact JSON: tables, analysis scripts, and \path{scripts/analysis/review_response_sensitivity.py} (see \path{REPRODUCE.md}). Replay of the 88-run campaign requires an NVIDIA A100-80GB, the listed weights, and \path{results/reports/runtime_manifest.json}. The driver version was not archived. Full chain-of-thought traces are not released.
...
\appendix
\section{Prompt}
MATH-500 and GSM8K use the QRM template, with a \texttt{<think>} prefix on the official QRM path:
\begin{verbatim}
{question}
...
answer within \boxed{}.
\end{verbatim}
GPQA-Diamond does not match that template in the stored rows. Across all 24 GPQA cells, $0$--$2$ completions per seed contain \texttt{\textbackslash boxed}, while pass@1 is about $41$--$53\%$ for every format, including BF16. Extractive match on GPQA is therefore not a boxed-answer score. The campaign was launched through official QRM \texttt{inference.py}. The prompt string itself is not stored in the compact JSON. The repository template \path{prompts/gpqa_diamond.txt} presents choices A--D and requests a final line \texttt{Answer: \$LETTER}. A local loader shuffles those choices with \texttt{Random(seed * 1000003 + item)}, which would keep choice order paired across checkpoints at a fixed seed. We do not claim that the QRM job used that shuffle, because the prompt text was not archived.
