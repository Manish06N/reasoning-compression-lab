...
\title{\textbf{One Stack, Many Rankings: Measuring Evaluation-Target Instability in Quantized Reasoning Checkpoints}}
...
\texttt{Midhunchakkaravarthy@lincoln.edu.my}}
...
\begin{abstract}
Practitioners often select a public quantized reasoning checkpoint using one published metric, typically pass@1. Prior measurement studies examine individual dimensions---accuracy~\cite{liu2025qrm}, serving throughput~\cite{kurtic2025give}, token length~\cite{lian2026quantinflates}, or calibration---but do not ask whether checkpoint rankings remain stable when the serving stack is frozen and the evaluation target changes.

We pin one stack and vary only the public weight checkpoint. Eight DeepSeek-R1-Distill checkpoints (BF16; FP8 executed as Marlin W8A16~\cite{frantar2024marlin} rather than native W8A8; AWQ-4; GPTQ-4; Qwen-7B and Llama-8B) are evaluated on an NVIDIA A100-80GB with vLLM~0.7.0~\cite{kwon2023vllm} eager execution. The campaign comprises 88 checkpoint$\times$benchmark$\times$seed runs and 56{,}408 completions on MATH-500~\cite{hendrycks2021math,lightman2023lets} (5 seeds), GSM8K~\cite{cobbe2021gsm8k} (3 seeds), and GPQA-Diamond~\cite{rein2023gpqa} (3 seeds).
...
\begin{itemize}
    \item \textbf{RQ1.} Do the evaluated quantized checkpoints differ in \emph{pass@1} from matched BF16, with problem-clustered uncertainty?
    ...
    \item \textbf{RQ3.} What can observable multi-sample agreement say about selective abstention without gold labels at serve time?
...
(C1)~\textbf{Pinned evaluation protocol.} NVIDIA A100-80GB; vLLM~0.7.0 eager; public DeepSeek-R1-Distill checkpoints; BF16; FP8 executed as Marlin W8A16; AWQ-4; GPTQ-4; problem-clustered bootstrap; Holm correction; TOST $\pm 1$\,pp; and an explicit pass@1 versus maj@5 distinction.
(C2)~\textbf{Ranking instability.} Under the same pinned stack, checkpoint rankings disagree across pass@1, maj@5, correctness-conditioned length, the historical token proxy, sequential GPU-seconds, and batched GPU-seconds.
(C3)~\textbf{Checkpoint-not-method result.} The tested community AWQ artifacts showed task-specific degradation. That finding applies to the tested public artifacts; it does not establish a universal property of AWQ. The released grid and analysis artifacts support reproducibility of these contributions.
...
Existing studies evaluate quantization accuracy, throughput, or individual reasoning behaviors. Our question is different: after fixing the serving stack, do practitioners receive the same checkpoint recommendation when the evaluation target changes? We study ranking stability rather than proposing another quantization method. Appendix Table~\ref{tab:related} places this measurement against those studies. The unit of comparison is the public checkpoint $\times$ pinned stack $\times$ evaluation target, not a quantization method in isolation. Numerically, QRM reports Qwen-7B MATH BF16 $94.60$ / AWQ $91.00$ and GPQA AWQ $50.00$; this pinned grid is $93.12$ / $44.78$ for the tested Qwen AWQ-4 checkpoint. We interpret that discrepancy as checkpoint- and stack-sensitive measurement, not as a superiority claim. The FP8 and GPTQ-4 cells are public RedHatAI compressed-tensors artifacts evaluated on A100 Marlin W8A16; we do not treat the measurements as Red Hat production-deployment claims. Complementary objects we do not claim to replace include SmoothQuant~\cite{xiao2023smoothquant} (not run), Lotfi et al.~\cite{lotfi2026overthinking}, Li et al.~\cite{li2025quantmeets}, Helcig et al.~\cite{helcig2026slq}, Zhong et al.~\cite{zhong2025quantized}, Ayadi et al.~\cite{ayadi2026reliability}, self-consistency~\cite{wang2022selfconsistency}, and G-Pass@$k$~\cite{liu2025gpass}. We report a historical shared-$65$ tok/s token-implied aggregate cost proxy and an aggregate hybrid Cost-of-Pass proxy; neither is Erol et al.'s per-problem estimator.
...
Table~\ref{tab:multitask} gives cell means; Tables~\ref{tab:gsm-contrasts} and~\ref{tab:gpqa-contrasts} are the clustered tests. FP8 stays within about one point of BF16 on all three tasks; several GSM8K Qwen (and Llama FP8/GPTQ) contrasts pass TOST at $\pm 1$\,pp. The tested Llama AWQ-4 checkpoint also showed a GSM8K drop ($-1.57$\,pp, $95\%$ CI $[-2.55,-0.58]$, $p=0.002$). The tested Qwen AWQ artifact exhibited a $5.56$ percentage point GPQA-Diamond difference under the primary Holm-6 family; this contrast does not remain significant under the Holm-18 joint sensitivity analysis (Appendix Table~\ref{tab:holm18}; $95\%$ CI $[-9.60,-1.52]$, $p=0.007$; 3 seeds, $n=198$). Compact-JSON item counts show that GPQA difference is distributed across items rather than concentrated in a few problems: 75 of 198 GPQA items flip on at least one seed for the tested Qwen AWQ-4 artifact, and no item is BF16-correct / AWQ-wrong on all three seeds. GPQA intervals are otherwise wide; we treat non-AWQ GPQA contrasts as exploratory. This GPQA contrast is not a headline result of the study.
...
\begin{table}[t]
\centering
\caption{GPQA-Diamond clustered pass@1 versus matched BF16 ($n=198$, 3 seeds). Holm over six contrasts. sig.: Holm-significant at $\alpha=0.05$; n.s.: not significant. The tested Qwen AWQ-4 row is reported with Holm-6 and Holm-18 joint sensitivity caveats (Appendix Table~\ref{tab:holm18}); it is not a headline claim.}
...
Table~\ref{tab:difficulty}: Level~1 is easy for every format in this sample. Among the evaluated Llama cells, the tested Llama AWQ-4 checkpoint showed the lowest pass@1 on Levels 2--4. MATH level labels are not a monotone difficulty axis here (Level~2 Qwen BF16 $89.11\%$ vs Level~5 $92.69\%$). These level splits are descriptive associations, not a causal difficulty mechanism.
...
\addplot[only marks, mark=*, blue, mark size=3pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    5.956865 94.00 0.103286 0.114559 \\
    };
\addplot[only marks, mark=square*, green!60!black, mark size=3pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    3.814391 94.40 0.671988 0.660611 \\
    };
\addplot[only marks, mark=triangle*, red, mark size=3.5pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    3.499199 93.12 0.064740 0.070019 \\
    };
\addplot[only marks, mark=diamond*, orange, mark size=3.5pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    3.222761 93.48 0.058101 0.063274 \\
    };
...
\addplot[only marks, mark=*, blue, mark size=3pt,
    ...
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    4.728241 89.24 0.106368 0.115058 \\
    };
\addplot[only marks, mark=square*, green!60!black, mark size=3pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    4.340136 89.52 0.098358 0.107171 \\
    };
\addplot[only marks, mark=triangle*, red, mark size=3.5pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    5.604882 86.48 0.139051 0.151922 \\
    };
\addplot[only marks, mark=diamond*, orange, mark size=3.5pt,
    error bars/.cd, x dir=both, x explicit, y dir=none]
    table[row sep=\\, x=x, y=y, x error minus=xm, x error plus=xp] {
    x y xm xp \\
    5.905101 88.92 0.130589 0.141440 \\
    };
\end{groupplot}
\end{tikzpicture}
\caption{Condition~B cost-accuracy scatter: aggregate hybrid Cost-of-Pass proxy versus campaign MATH-500 pass@1 (\$ $\times 10^{-3}$). Whiskers are 95\% uncertainty intervals conditional on the observed timing repetitions and clustered accuracy bootstrap. Points are not joined. Panels represent separate serving conditions (Qwen-7B on \texttt{ragpu003}; Llama-8B on \texttt{ragpu004}) and are not a pooled trade-off plot. Qwen FP8's wide whisker is the five-rep wall-clock spread.}
...
\end{axis}
\end{tikzpicture}
\caption{Full-grid mean MATH-500 completion tokens (ratio-of-means estimand). Bars are point estimates; clustered 95\% intervals on paired token $\Delta$ are in Table~\ref{tab:tokens} and are not drawn on this axis.}
\label{fig:tokens}
\end{figure}
...
A higher token count is not automatically cancelled by a higher measured tok/s. Cost is GPU-seconds $\times$ a pricing scenario, not tokens alone. No serving-confirmation \emph{mean} output reached the $32{,}768$-token generation cap. Compact confirmation JSON does not store native \texttt{finish\_reason}.
...
Figure~\ref{fig:condB-scatter} is a Condition~B cost-accuracy scatter. Panels represent separate serving conditions (different hosts and model families) and are not a pooled trade-off plot. Latency detail for Condition~A remains in Appendix Table~\ref{tab:serving-single}.
...
\section{Discussion}
Under this pinned A100 W8A16 stack, evaluating these eight public checkpoints requires more than a single pass@1 or token count. Rank order among the tested checkpoints depends on the evaluation target. MATH-500 FP8--BF16 intervals include zero, but TOST at $\pm 1$\,pp fails, so we do not treat the tested FP8 checkpoints as a statistically equivalent accuracy default. Observed 4-bit behavior is checkpoint- and task-specific: the tested Qwen AWQ-4 checkpoint keeps MATH-500 and GSM8K near BF16, while the tested Llama AWQ-4 checkpoint shows the largest MATH-500 and GSM8K pass@1 drops among the evaluated Llama cells. The tested Qwen AWQ artifact exhibited a $5.56$ percentage point GPQA-Diamond difference under the primary Holm-6 family; this contrast does not remain significant under the Holm-18 joint sensitivity analysis and is not treated as a headline result. The tested Qwen 4-bit checkpoints showed $6.3$--$6.9\%$ higher mean MATH-500 length. Both-OK CIs exclude zero for those Qwen 4-bit cells, so jointly-correct traces were still longer. Mismatch-conditioned differences are much larger because failure traces are themselves long; $D$ is a diagnostic of that asymmetry, not a causal compression effect.
...
\begin{enumerate}
    ...
    \item \textbf{Checkpoint provenance.} AWQ-4 is a community quantization of unknown calibration set and quantizer revision. Accuracy losses on the tested community AWQ artifacts are properties of those artifacts, not a proof about AWQ as a method. GPTQ-4/FP8 are RedHatAI artifacts (appendix).
    ...
    \item \textbf{Reproducibility surface.} Compact per-cell JSON, recovered MATH-500 answer strings, confirmation timing JSON, and analysis reports are released. Full GPU traces (chain-of-thought text) are not publicly released. Compact campaign JSON also omits \texttt{finish\_reason} and output token IDs. Table consistency is verified automatically against frozen expected values; the \LaTeX\ source remains manually formatted for publication layout. Replaying the complete GPU campaign requires the original A100 hardware, checkpoint weights, and dataset revisions; the protocol is inspectable and is not expected to be rerun by every reviewer. The NVIDIA driver version was unavailable in the archived environment and is recorded as UNRECORDED in \texttt{runtime\_manifest.json}.
...
CPU-reproducible from compact JSON: tables, analysis scripts, statistical checks,
and serving-confirmation analysis (see \path{REPRODUCE.md}). Table consistency is verified automatically against frozen expected values; the \LaTeX\ source remains manually formatted for publication layout. Replay of the 88-run campaign requires an NVIDIA A100-80GB GPU, the listed model weights, and an HPC-class environment matching \path{results/reports/runtime_manifest.json}. The NVIDIA driver version was unavailable in the archived environment. Full chain-of-thought traces are not released.
...
\label{app:holm18}
Primary multiplicity control is Holm--Bonferroni within each benchmark's six format contrasts. Table~\ref{tab:holm18} applies Holm jointly to all 18 contrasts as a stricter sensitivity. Llama MATH AWQ-4 and Llama GSM8K AWQ-4 remain significant. The tested Qwen AWQ artifact exhibited a $5.56$ percentage point GPQA-Diamond difference under the primary Holm-6 family; this contrast does not remain significant under the Holm-18 joint sensitivity analysis (Holm-adjusted $p=0.109$). That does not change the primary within-benchmark inference.
...
\path{results/measured_serving/} used unpinned \path{max_num_seqs}, an unbalanced Condition~A first-20 draw, and mixed-node Llama repeats. Absolute tok/s from that run is not comparable to the confirmation after pinning concurrency 8. Those files are provenance; they are not averaged with confirmation and are not cited in the abstract.

\paragraph{Pathology key mismatch.}
Earlier drafts reported $0/0$ loops and cap hits because analysis scripts read \path{truncation_count} / \path{repetition_flag_count} while the validator writes \path{token_limit_hits} / \path{repetition_rows}. Canonical counts are 25 loops and 0 exact cap hits over 56{,}408 completions (Section~\ref{sec:pathology}).
