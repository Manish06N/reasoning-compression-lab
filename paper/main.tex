...
\section{Conclusion}
Under this pinned A100 / vLLM~0.7.0 eager stack, the defensible results are one robust accuracy drop, a placebo-sized selection effect in the BF16-correct length estimator, a gold-free agreement signal whose benefit arrives mostly with the second sample, and a subset serving cost that one length draw can reorder. Llama AWQ-4 falls $-2.76$\,pp on MATH-500; maj@5 cuts that gap to about $-1.4$\,pp, and the drop is concentrated at difficulty levels 3--5. A BF16-versus-BF16 placebo reproduces $+138$ to $+235$ tokens of the BF16-correct length increase, and the quant-correct mirror does not keep its sign. Two-sample unanimity has $0.7$--$2.4\%$ selective risk against $2.2$--$5.8\%$ for a matched one-sample length rule; 5/5 agreement reaches at most $0.27\%$. Llama FP8 is cheapest once campaign length replaces the 20-prompt draw. AWQ-4 results are properties of the \texttt{jakiAJK} artifacts---their chat template, \texttt{float16} activations, and the configured GEMM \texttt{awq} path. The GPQA AWQ-4 contrast is paired but remains borderline.
