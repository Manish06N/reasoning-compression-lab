...
Under this pin, estimands disagree. On MATH-500, FP8–BF16 pass@1 differences are $+0.40$ and $+0.28$ percentage points (pp); problem-clustered 95% intervals include zero, and a $\pm 1$ pp TOST test is not passed. The tested Qwen AWQ-4 checkpoint loses $5.56$ pp on GPQA-Diamond (Holm-significant within the six GPQA contrasts, not under a joint 18-contrast Holm sensitivity). Mean MATH-500 length rises $6.3$–$6.9\%$ for Qwen 4-bit; jointly-correct Qwen AWQ-4/GPTQ-4 traces also lengthen, and the clustered mismatch-excess $D$ is positive with intervals excluding zero across all six MATH contrasts. BF16 failures themselves are long, so those length contrasts are not interpreted causally. Strict five-sample unique-mode agreement yields low observed selective risk, trading off coverage and requiring five generations. Token-proxy, sequential Condition A, and batched Condition B produce different rankings: the proxy ranks Qwen FP8 first among Qwen cells, Condition A ranks Qwen AWQ-4 first, and Condition B ranks Qwen GPTQ-4 first. Effects and rankings are checkpoint-, task-, estimand-, and serving-regime-specific under this pinned stack.
...
| Qwen AWQ-4 | Tested checkpoint: −5.56 pp GPQA-Diamond pass@1; Holm-significant within the six GPQA contrasts, not under Holm-18 |
| Qwen 4-bit tokens | $+6.3$–$6.9\%$ RoM vs BF16; Both-OK CIs exclude 0; mismatch excess $D$ excludes 0 in all six MATH contrasts; BF16-correct conditional $\Delta$, following Lian et al., positive |
